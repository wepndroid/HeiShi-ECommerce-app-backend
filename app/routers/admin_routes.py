...
def _listing_admin_summary(row: Listing) -> dict:
    return {
        "id": row.id,
        "type": row.type,
        "title": row.title,
...
def _listing_admin_detail(db: Session, listing: Listing) -> dict:
    ...
    return {
        **_listing_admin_summary(listing),
...
def dashboard_stats(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    return {
        ...
        "disputeOrderCount": db.query(func.count(Order.id)).filter(Order.status == "inDispute").scalar() or 0,
        "dau": dau_row.user_count if dau_row else 0,
        "promotionClicks": db.query(func.coalesce(func.sum(Listing.promotion_click_count), 0)).scalar() or 0,
...
def list_content(
    ...
    rows = q.offset((page - 1) * pageSize).limit(pageSize).all()
    return {
        "items": [_listing_admin_summary(row) for row in rows],
        "total": total,
        "page": page,
        "pageSize": pageSize,
    }
